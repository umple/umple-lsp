...
- npm publish is a hard-to-reverse action with global blast radius, so a human still bumps the committed package version and approves the environment. The workflow input can usually be left blank; it publishes the version from `packages/server/package.json`.
